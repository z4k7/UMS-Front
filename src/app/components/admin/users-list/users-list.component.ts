...
import { userSelector } from 'src/app/states/user/user.selectors';
...
export class UsersListComponent implements OnInit {
  ...
  userData$ = this.store.pipe(select(userSelector))
  ...
  confirmDelete(userId:string){
    Swal.fire({
      ...
      if(result.isConfirmed){
        ...
        this.http.post(`admin/deleteUser/${userId}`,{},{withCredentials:true}).subscribe({
          next:()=>{
            this.store.dispatch(retrieveUsers())
            Swal.fire(
              'Deleted!',
